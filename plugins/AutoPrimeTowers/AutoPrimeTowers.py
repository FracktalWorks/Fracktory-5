...
catalog = i18nCatalog("AutoPrimeTowers")

class AutoPrimeTowersPlugin(Extension):
    def __init__(self):
        super().__init__()
        ...
        self.addMenuItem(catalog.i18n("Recalculate Prime Towers"), self.updatePrimeTowerSize)
        self.addMenuItem(catalog.i18n("Enable Auto Prime Towers"), self.enablePlugin)
        self.addMenuItem(catalog.i18n("Disable Auto Prime Towers"), self.disablePlugin)
    ...
    def _debounceUpdatePrimeTowerSize(self, delay=1):
    ...
    def onSettingChanged(self, key, value):
        if not self._enabled:
            return
        # Update prime tower on any setting change
        self.updatePrimeTowerSize()
    ...
    def showUserMessage(self, text):
        if self._message:
            self._message.hide()
        self._message = Message(text, title=catalog.i18n("AutoPrimeTowers"))
        self._message.show()
